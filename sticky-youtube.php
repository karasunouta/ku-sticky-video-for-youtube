...
class Sticky_YouTube {
	...
	public function add_settings_page() {
		add_options_page(
			__( 'Sticky YouTube Settings', 'sticky-youtube' ),
			'Sticky YouTube',
			'manage_options',
			$this->slug,
	...
	public function register_settings() {
		register_setting(
			'sticky-youtube-options-group',
			'sticky-youtube-options',
			array(
	...
	public function admin_enqueue_scripts( $hook ) {
		// Sticky YouTubeの設定ページ以外ではスクリプト・スタイルを読み込まない
		if ( 'settings_page_' . $this->slug !== $hook ) {
			return;
		}

		// パスの整理
		$entry_point    = 'sticky-youtube-admin';
		$asset_path     = plugin_dir_path( __FILE__ ) . "build/{$entry_point}.asset.php";
		$script_url     = plugins_url( "/build/{$entry_point}.js", __FILE__ );
		$style_url      = plugins_url( "/build/{$entry_point}.css", __FILE__ );
		$style_path     = plugin_dir_path( __FILE__ ) . "build/{$entry_point}.css";
		...
		// JS変数のセット（将来的に設定値などを渡す必要がある場合に備え、雛形として残します）
		wp_localize_script(
			$script_handle,
			'stickyYouTubeAdminSettings',
			array(
	...
	public function render_settings_page() {
		...
		<div class="wrap sticky-youtube-admin-wrap">
			...
			<div class="sticky-youtube-header">
				<h1>Sticky YouTube <span><?php echo esc_html( self::VERSION ); ?></span></h1>
				<p><?php esc_html_e( 'Configure options for your floating YouTube video player.', 'sticky-youtube' ); ?></p>
			</div>

			<?php
			if ( isset( $_GET['settings-updated'] ) && 'true' === $_GET['settings-updated'] ) {
				echo '<div class="success-badge"><svg width="14" height="14" fill="currentColor" viewBox="0 0 20 20" style="margin-right: 4px;"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>' . esc_html__( 'Settings saved successfully.', 'sticky-youtube' ) . '</div>';
			}
			?>

			<form method="post" action="options.php">
				<?php settings_fields( 'sticky-youtube-options-group' ); ?>

				<div class="sticky-youtube-card">
					<h2 class="sticky-youtube-card-title">
						<svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
							<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4"></path>
						</svg>
						<?php esc_html_e( 'General Settings', 'sticky-youtube' ); ?>
					</h2>

					<div class="form-group">
						<label for="exclude_class"><?php esc_html_e( 'Exclude CSS Class Name', 'sticky-youtube' ); ?></label>
						<div class="input-wrapper">
							<span class="prefix-dot">.</span>
							<input type="text" id="exclude_class" name="sticky-youtube-options[exclude_class]" value="<?php echo esc_attr( $exclude_class ); ?>" class="input-field" placeholder="no-sticky" />
						</div>
					...
					<div class="instruction-box">
						<h3><?php esc_html_e( 'How to Exclude a YouTube Block', 'sticky-youtube' ); ?></h3>
						<ol>
				...
				<button type="submit" class="submit-btn">
					...
					<?php esc_html_e( 'Save Changes', 'sticky-youtube' ); ?>
				</button>
			</form>
		</div>
		<?php
